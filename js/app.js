      ...
      async function generateHistory() {
        try {
          ...
          acfData.historique = data.success
            ...
            : [];
          console.log(
            `📋 ${acfData.historique.length} entrées d'historique chargées`
          );
        } catch (e) {
...
function switchSection(section) {
  ...
  const titles = {
    identite: {
      ...
      desc: "Gérez les éléments d'identité visuelle de votre site",
    },
    header: {
      title: "Header",
      desc: "Modifiez les éléments du header de votre site",
    },
    presentation: {
      title: "Présentation",
      desc: "Gérez la section présentation de votre entreprise",
    },
    partenaires: {
      title: "Partenaires",
      desc: "Administrez vos partenaires et leurs informations",
    },
    services: {
      title: "Services",
      desc: "Modifiez la présentation de vos services",
    },
    carrousel: {
      title: "Carrousel",
      desc: "Gérez les images et contenus du carrousel",
    },
    contact: { title: "Contact", desc: "Modifiez les informations de contact" },
    historique: {
      title: "Historique",
      desc: "Consultez l'historique des modifications",
    },
  };
              ...
              .map(
                ...
                <div class="history-item">
                  ...
                  item.field
                        ...
                        <div class="history-time">${formatDate(
                          item.timestamp
                        )} par ${item.user}</div>
                    </div>
                </div>
...
function handleGlobalSearch(e) {
  ...
  const results = allFields.filter(
    (field) =>
...
function renderSearchResults(results, searchTerm) {
  ...
  if (descEl)
    descEl.textContent = `${results.length} résultat(s) pour "${searchTerm}"`;
              ...
              .map(
                ...
                <div class="col-md-6 col-lg-4">
                    <div class="field-card">
                        ...
                        <h5 class="field-title">${highlightText(
                          field.title,
                          searchTerm
                        )}</h5>
                        <div class="field-last-modified">
